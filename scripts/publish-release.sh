#!/bin/zsh
# Create GitHub release vX.Y.Z only when both thin architecture DMGs exist.
...
amd64_dmg="$ROOT/build/bin/darwin-amd64/IPARenewalAssistant-${TAG}-macos-amd64.dmg"
arm64_dmg="$ROOT/build/bin/darwin-arm64/IPARenewalAssistant-${TAG}-macos-arm64.dmg"

[[ -f "$amd64_dmg" ]] || fail "缺少 Intel 包：$amd64_dmg"
[[ -f "$arm64_dmg" ]] || fail "缺少 Apple Silicon 包：$arm64_dmg。请在 M 芯片 Mac 上运行 ./scripts/build-app.sh arm64，完成真机验收后再发版。"
...
[[ -d "$arm64_app" ]] || fail "缺少 Apple Silicon .app：$arm64_app。DMG 存在但未提供可核验的安装包。"
...
cp "$amd64_dmg" "$arm64_dmg" "$WORKDIR/"
(
  cd "$WORKDIR"
  shasum -a 256 IPARenewalAssistant-${TAG}-macos-amd64.dmg IPARenewalAssistant-${TAG}-macos-arm64.dmg > SHA256SUMS.txt
)
...
gh release create "$TAG" \
  ...
  --title "续签助手 ${TAG}" \
  --notes-file "$NOTES" \
  "$WORKDIR/IPARenewalAssistant-${TAG}-macos-amd64.dmg" \
  "$WORKDIR/IPARenewalAssistant-${TAG}-macos-arm64.dmg" \
...
print -- "$view" | grep -q "IPARenewalAssistant-${TAG}-macos-amd64.dmg" || fail "缺少 amd64 DMG 资产"
print -- "$view" | grep -q "IPARenewalAssistant-${TAG}-macos-arm64.dmg" || fail "缺少 arm64 DMG 资产"
