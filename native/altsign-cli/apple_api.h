...
@property (nonatomic, copy) NSString *identifier;   // Developer services resource id
...
/// Hex serial numbers with leading zeros removed, for matching Apple CSR replies.
FOUNDATION_EXPORT NSString *ALTNormalizedCertificateSerial(NSString * _Nullable value);

/// Looks up an extractable private key in the default keychain for a team certificate.
FOUNDATION_EXPORT NSData * _Nullable ALTCopyPrivateKeyFromSystemKeychain(ALTCertificate *certificate);

/// Finds an Apple Development identity for the team in the default keychain.
FOUNDATION_EXPORT NSData * _Nullable ALTCopyTeamIdentityFromSystemKeychain(NSString *teamID, ALTCertificate * _Nullable * _Nullable outCertificate);
